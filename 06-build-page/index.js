...
let readCssStream,writeCssStream;
...
fs.unlink(path.join(__dirname, 'project-dist','style.css'), function(err){  //--Удаление старого CSS файла
});


function deleteDir(files,pathCopy) {    //--------------------------------------Удаление старых директорий
    fs.rmdir(path.join(__dirname, 'project-dist','assets'), err => {
     });
}
function copy(files,pathCopy) {  //---------------------------------------------Копирование файлов и директорий
    fs.mkdir(path.join(__dirname,'project-dist', pathCopy), err => {});
    files.forEach(file => {
